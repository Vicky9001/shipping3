<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="发件人" min-width="180" align="center">
        <template slot-scope="{row}">
          <span>{{ row.shipper }}</span>
        </template>
      </el-table-column>
      <el-table-column label="收件人" min-width="180" align="center">
        <template slot-scope="{row}">
      ...
      <el-table-column label="操作" prop="operation" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-button size="small" :disabled="row.state !== 0" @click="handleAllocate(row)">
            分配车辆和运货员
          </el-button>
        </template>
      </el-table-column>
    ...
    <el-dialog :title="'分配车辆和承运人'" :visible.sync="dialogFormVisible">
      <el-form ref="update" :rules="rules" :model="update" label-position="left" label-width="120px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item label="承运人" prop="carriers">
          <el-select v-model="update.carriers" placeholder="请选择承运人" multiple>
            <el-option v-for="carrier in temp.carriers" :key="carrier.id" :label="carrier.realName" :value="carrier.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="运输车辆" prop="carriers">
          <el-select v-model="update.vans" placeholder="请选择运输车辆" multiple>
            <el-option v-for="van in temp.vans" :key="van.id" :label="`${van.license} (最大载重：${van.maxLoad} kg)`" :value="van.id" />
          </el-select>
        </el-form-item>
        <el-form-item label="运费/元" prop="money">
          <el-input v-model.number="update.money" type="number" step="0.01" min="0" placeholder="请输入运费" />
        </el-form-item>
      </el-form>
...
import { getOrder, getShipment, getDetail, allocateCVW } from '@/api/order'
import { getCarrierList } from '@/api/user'
import { getVanList } from '@/api/van'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        carriers: [
          { required: true, message: '请选择承运人', trigger: 'blur' }
        ],
        vans: [
        ...
        money: [
          ...
          { type: 'number', message: '运费必须为数字值', trigger: 'blur' }
        ]
      },
      currentPage: 1, // 当前页码
      total: 20, // 总条数
  ...
  methods: {
    ...
    handleAllocate(row) {
      ...
      getCarrierList({ 'userName': '', 'realName': '' }).then(response => {
        this.temp.carriers = []
        response.data.carrierList.forEach(item => {
          const data = {}
          data.id = item.id
          data.realName = item.realName
          this.temp.carriers.push(data)
        })
      })
      getVanList({ 'license': '', 'state': '0' }).then(response => {
        this.temp.vans = []
    ...
    allocate() {
      this.$refs.update.validate(valid => {
        if (valid) {
          // 表单验证通过，继续执行分配车辆和承运人的逻辑
          const vans = this.update.vans // 选中的运输车辆
          const carriers = this.update.carriers // 选中的承运人
          let maxLoadSum = 0 // 运输车辆的maxLoad之和
          vans.forEach(i => {
            this.temp.vans.forEach(item => {
              if (item.id === i) {
                maxLoadSum = maxLoadSum + item.maxLoad
              }
            })
          })
          const maxCars = carriers.length // 承运人的数量
          if (vans.length > maxCars) {
            this.$message.error('运输车辆数量不能大于承运人数量')
            return
          }
          if (maxLoadSum < this.temp.weight) {
            this.$message.error(`运输车辆的最大载重之和不能小于${this.temp.weight}`)
            return
          }
          // 继续执行分配车辆和承运人的逻辑
          allocateCVW({ carrierIds: this.update.carriers,
            id: this.update.id,
            vanIds: this.update.vans,
            money: this.update.money }).then(response => {
            this.getList()
            this.update = {
              id: '',
              carriers: [],
